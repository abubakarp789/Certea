...
def validate_file_path(
    filepath: str, must_exist: bool = False, allow_create: bool = True
) -> str:
    ...
    try:
        ...
        # Disallow .. entirely to prevent path traversal
        ...
        # Restrict to current working directory and its subdirectories
        cwd = os.getcwd()
...
def validate_directory_path(
    dirpath: str, must_exist: bool = False, create_if_missing: bool = False
) -> str:
...
def validate_key_size(key_size: int) -> int:
    ...
    if key_size % 256 != 0:
        raise ValidationError(f"Key size should be a multiple of 256, got {key_size}")

    return key_size


def validate_certificate_dates(
    valid_from: datetime, valid_until: datetime, max_validity_days: Optional[int] = None
    ...
    if valid_until <= valid_from:
        raise ValidationError("Certificate valid_until must be after valid_from")
...
def validate_validity_days(days: int) -> int:
    ...
    if days > 36500:  # ~100 years
        raise ValidationError(f"Validity days is too large (maximum 36500), got {days}")

    return days


def sanitize_string_input(
    input_str: str, max_length: int = 1000, allow_newlines: bool = False
) -> str:
...
def validate_request_size(
    content_length: int, max_size_mb: Optional[int] = None
) -> int:
    """
